...
def setup_routes(app, client, tool_data, assistant_id):
    ...
    def receive_sms():
        ...
        db_entry = core_functions.get_chat_mapping("twilio", from_number, assistant_id)
        ...
        if not thread_id:
            ...
            core_functions.update_chat_mapping(
                "twilio", from_number, assistant_id, thread.id
            )
        ...
        twilio_client.messages.create(
            body=response, from_=TWILIO_PHONE_NUMBER, to=from_number
        )
    ...
    def receive_whatsapp():
        ...
        if not thread_id:
            ...
            core_functions.update_chat_mapping(
                "twilio", from_number, assistant_id, thread_id
            )
        ...
        # Send the response back to the user's phone number
        twilio_client.messages.create(
            body=response, from_="whatsapp:" + TWILIO_PHONE_NUMBER, to=from_number
        )
